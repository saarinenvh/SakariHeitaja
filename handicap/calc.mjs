...
class HandicappedScores {
        ...
        try {
          if (newData.Competition != null) {
              for (let player of competition.players) {
                  let playerData = newData.Competition.Results.find(n => n.Name == player.name);
  ...
  getHandicappedResults(args, chatId) {
    ...
    switch(args) {
      case "kaatis":
        text = this.createCompetitionMessage(competitions.find(n => n.competitionName == "Kaatis"))
        break;
      case "karkkila":
        text = this.createCompetitionMessage(competitions.find(n => n.competitionName == "Karkkila"))
        break;
      case "kisis":
        text = this.createCompetitionMessage(competitions.find(n => n.competitionName == "Kisis"))
        break;
      case "hamis":
        text = this.createCompetitionMessage(competitions.find(n => n.competitionName == "Hamis"))
      break;
      case "all":
      console.log("asdf")
        text = "";
  ...
  createCompetitionMessage(competition) {
    ...
    text += `<code>${this.fillString("Sija, Nimi", 22)} ${this.fillString("Tulos", 5)} ${this.fillString("Pisteet", 7)}</code>\n`
    competition.players.forEach(n => text += `<code>${this.fillString(n.rank + ". " + n.name, 22)} ${this.fillString(n.handicappedScore.toString(), 5)} ${this.fillString(n.points.toString() + "pst", 7)} </code>\n`)
    return text;
  }
  ...
  createOverallMessage() {
    ...
    overallText += `<code>${this.fillString("Sija, Nimi", 22)} ${this.fillString("Pisteet", 7)}</code>\n`
    overall.forEach((n, index) => overallText += `<code>${this.fillString( (index + 1).toString() + ". " + n.name, 22)} ${this.fillString(n.points + "pst", 7)}\n</code>`)
    return overallText
  }
