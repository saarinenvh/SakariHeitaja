...
/**
 * Commentary engine — generates Finnish-language commentary for score events.
 *
 * This is intentionally isolated so it can be swapped for Ollama AI later
 * without touching the orchestrator or any other module.
 *
 * Public API:
 *   generateComment(change) → string
...
const scoreTexts = {
  ...
  worse:        (score) => [`niin ison scoren, että ei mahdu edes näytölle (${score})`],
};
...
function addPlusSign(score) {
  return score > 0 ? `+${score}` : `${score}`;
}

function getScoreType(holeResult) {
...
function getScoreText(type, holeResult) {
  if (type === "worse") return pick(scoreTexts.worse(holeResult.Result));
  return pick(scoreTexts[type]);
}

function getStartText(type) {
...
/**
 * Generates a Finnish commentary string for a single score change.
 *
 * @param {object} change  — from changeDetector.detectChanges()
 * @returns {string}
 */
export function generateComment(change) {
  const { newPlayer, hole, holeResult } = change;
...
/**
 * Generates a random round header narrative.
 *
 * @returns {string}
 */
